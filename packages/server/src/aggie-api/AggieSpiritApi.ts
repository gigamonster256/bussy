...
import { AggieSpiritAuth } from "./AggieSpiritAuth";
  ...
  effect: Effect.gen(function* () {
    ...
    return {
      getBaseData: Effect.fn("AggieSpiritApi.getBaseData")(function* () {
        ...
        return raw.routes.map(
          (r): Route => ({
            id: r.key,
            name: r.name,
            shortName: r.shortName,
            directionList: r.directionList.map(
              (d): Direction => ({
                id: d.direction.key,
                name: d.direction.name,
                destination: d.destination,
                patternList: d.patternList.map(
                  (p): Pattern => ({
                    id: p.key,
      ...
      getPatternPaths: Effect.fn("AggieSpiritApi.getPatternPaths")(function* (
        routeID: string,
      ...
      getNextDepartureTimes: Effect.fn("AggieSpiritApi.getNextDepartureTimes")(function* (
        routeID: string,
        directionIDs: Array<string>,
        stopCode: string,
      ) {
        const headers = yield* auth.headers();
        const raw = (yield* Effect.promise(() =>
          ...
          Effect.withSpan("aggie-spirit-api.getNextDepartureTimes", {
            attributes: { routeID, stopCode },
          }),
